...
def load_client_data(
    ...
    centralized_val_row_ids: List[int] | None = None,
    ...
    # 2. Optional: zentralisiertes Validation-Set verwendet immer dieselben Row-IDs.
    effective_val_row_ids = (
        centralized_val_row_ids if centralized_val_row_ids is not None else val_row_ids
    )

    # 3. Lade NUR Client-Rows (PyArrow Filter - sehr schnell!)
    all_row_ids = set(train_row_ids + effective_val_row_ids)
    ...
    # 4. Extrahiere Features & Labels (KEINE Normalisierung mehr!)
    ...
    val_indices = [row_id_to_idx[int(rid)] for rid in effective_val_row_ids]
    ...
    # 7. Class-Weights aus norm_stats.json (vorberechnet, weight bosst für pos schon drin 1,5)
...
def load_centralized_val(
    ...
    """Lade nur Val-Daten für Server-seitige Evaluation."""
    
    meta = json.loads(Path(stats_path).read_text())
    
    df = pd.read_parquet(
        parquet_path,
        filters=[("__row_id__", "in", val_row_ids)]
    )
    
    target_col = meta["target"]
    y = df[target_col].astype(int).values
    X = df.drop(columns=[target_col, "__row_id__"]).values.astype("float32")
