...
VARIANT_PALETTE = [
    ...
    "#0271BB",  # UHH blue
    "#3B515B",  # UHH slate
    "#8241A8",  # complementary violet
    "#FF962D",  # complementary ochre
...
def plot_family_panels(
    ...
    """Create an MCC panel followed by one threshold panel per run."""
    ensure_outdir(outdir)
    ...
    variants = sorted(
        int(v) for v in family_df["variant"].dropna().unique()
    )
    if not variants:
        raise ValueError(f"No variants found for family {family}")

    # One large MCC panel followed by one compact threshold panel per run.
    height_ratios = [3.4] + [0.62] * len(variants)
    effective_height = max(
        figure_height,
        5.2 + 0.72 * len(variants),
    )
    ...
    main_ax = axes[0]
    threshold_axes = axes[1:]

    legend_handles: List[Line2D] = []
    legend_labels: List[str] = []

    # Use identical threshold limits for all run-specific panels.
    threshold_values = pd.to_numeric(
        family_df["best_mcc_threshold"],
        errors="coerce",
    ).to_numpy()
    ...
    if finite_thresholds.size:
        threshold_min = (
            np.floor(
                finite_thresholds.min() / threshold_tick_interval
            )
            * threshold_tick_interval
        )
        threshold_max = (
            np.ceil(
                finite_thresholds.max() / threshold_tick_interval
            )
            * threshold_tick_interval
        )
        ...
        # Small margin so that the outer grid values do not lie on the border.
        threshold_ylim = (
            max(0.0, threshold_min - threshold_tick_interval / 2),
            min(1.0, threshold_max + threshold_tick_interval / 2),
        )
    else:
        threshold_ylim = (0.0, 1.0)

    for threshold_ax, variant in zip(threshold_axes, variants):
        group = (
            family_df[family_df["variant"] == variant]
            .sort_values("round")
        )
        if group.empty:
        ...
        rounds = group["round"].astype(int)
        mcc_values = group["best_mcc_value"].astype(float)
        thresholds = group["best_mcc_threshold"].astype(float)

        # Shared MCC comparison panel.
        main_ax.plot(
            rounds,
            mcc_values,
            color=color,
        ...
        # Separate threshold panel for this run.
        # A step line reflects the discrete 0.05 threshold grid and does not
        # visually suggest intermediate threshold values.
        threshold_ax.step(
            rounds,
            thresholds,
            where="mid",
        ...
        threshold_ax.set_ylim(*threshold_ylim)
        ...
        # Colored in-panel label identifies the run without another legend.
        threshold_ax.text(
            0.008,
            0.82,
            f"Run {variant}",
        ...
        if run_summary.empty:
            ...
            label = (
                ...
                f"{row['plateau_mcc_mean']:.3f} "
                f"± {row['plateau_mcc_std']:.3f}, "
                f"R{target_pct:.0f}={convergence}"
            )
    ...
    # Only the final threshold panel needs the x-axis label.
    threshold_axes[-1].set_xlabel(
        "Communication Round",
        fontsize=12,
    )
    ...
    for axis in axes:
        ...
        axis.xaxis.set_major_locator(
            mticker.MultipleLocator(x_tick_interval)
        )
    ...
    # Hide repeated x-axis labels above the final panel.
    main_ax.tick_params(labelbottom=False)
    for threshold_ax in threshold_axes[:-1]:
    ...
    # Shared label for all threshold panels.
    threshold_top = threshold_axes[0].get_position().y1
    threshold_bottom = threshold_axes[-1].get_position().y0
    threshold_center = (threshold_top + threshold_bottom) / 2
