#!/usr/bin/env python3
"""
Threshold-dependent FedAdam evaluation for validation-AP-selected checkpoints.
...
1. Loads the checkpoint already selected by highest validation Average Precision
   from:
       result/splits_iid_scaling/
         splits_iid_<N>_clients.json/FedAdam/bestPRROC/run_<r>/
...
4. Writes one compact file required by plot-thr-dependent-fedadam.py:
...
FedAdam validation and test data
--------------------------------
The centralized validation IDs are read from
split_data["centralized_val_row_ids"], matching scaling_eval_fixed.py and the
server-side validation pipeline. Their membership is verified against
norm_stats.json["val_idx"] for every scaling point. The centralized test set is
loaded once from norm_stats.json["test_idx"] and is used only after both
thresholds have been selected on the validation set.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Select exact validation thresholds for FedAdam bestPRROC "
...
def validation_ids_from_fedadam_split(
    split_root: Path,
    n_clients: int,
    expected_global_ids: Sequence[int],
) -> List[int]:
    """
    Read centralized_val_row_ids exactly as in scaling_eval_fixed.py and verify
    that the complete centralized validation membership is used.
    """
    split_path = split_root / f"splits_iid_{n_clients}_clients.json"
    payload = load_json(split_path)
    raw_ids = payload.get("centralized_val_row_ids")

    if not isinstance(raw_ids, list) or not raw_ids:
        raise KeyError(
            f"No non-empty 'centralized_val_row_ids' list in {split_path}"
        )

    validation_ids = [int(value) for value in raw_ids]
    if len(validation_ids) != len(set(validation_ids)):
        raise ValueError(
            f"Duplicate centralized validation IDs in {split_path}"
        )

    observed = set(validation_ids)
    ...
    if missing or unexpected:
        raise ValueError(
            f"Centralized validation membership mismatch for {n_clients} "
            f"clients: missing={len(missing)}, unexpected={len(unexpected)}."
        )

    print(
        f"   ✓ centralized_val_row_ids verified: "
        f"{len(validation_ids)} rows"
    )
    return validation_ids


def load_fedadam_validation_split(
    parquet_path: Path,
    stats_path: Path,
    ...
    """
    Load validation data through the same project function used by the FedAdam
    checkpoint-selection script.
    """
    features, labels, _class_weights = load_centralized_val(
        parquet_path=str(parquet_path),
        stats_path=str(stats_path),
        val_row_ids=[int(value) for value in row_ids],
    )

    features_array = np.asarray(features, dtype=np.float32)
    labels_array = np.asarray(labels, dtype=np.int64)
    labels_array = (labels_array >= 1).astype(np.int64, copy=False)

    if features_array.ndim != 2:
        raise ValueError(
            f"Expected a 2D validation feature matrix, got "
            f"shape {features_array.shape}."
        )
    if features_array.shape[0] != len(row_ids):
        raise ValueError(
            f"Loaded {features_array.shape[0]} validation rows, expected "
            f"{len(row_ids)}."
        )
    if features_array.shape[1] != expected_features:
        raise ValueError(
            f"Expected {expected_features} validation features, loaded "
            f"{features_array.shape[1]}."
        )
    if np.unique(labels_array).size != 2:
        raise ValueError(
            "The centralized validation set does not contain both classes."
        )

    dataset = TensorDataset(
        torch.as_tensor(features_array, dtype=torch.float32),
        torch.as_tensor(labels_array, dtype=torch.long),
    )
    loader = DataLoader(
        dataset,
    ...
    info = {
        ...
        "positive_prevalence": float(n_positive / len(labels_array)),
    }
    return loader, info
...
def discover_best_pr_checkpoint(
    ...
    # Verify that the stored AP selection used a finite metric and the complete validation set.
...
def main() -> None:
    ...
    print("=" * 84)
    print("THRESHOLD-DEPENDENT FedAdam EVALUATION")
    ...
    # Verify every split before evaluating any model and retain the first
    # validated centralized ID list for loading. Exact membership is required
    # at every scaling point.
    validated_val_ids: Optional[List[int]] = None
    for n_clients in args.scaling_points:
        print(f"\n🔎 Verifying {n_clients} clients")
        current_ids = validation_ids_from_fedadam_split(
            split_root=args.split_root,
            n_clients=n_clients,
            expected_global_ids=global_indices["val_idx"],
        )
        if validated_val_ids is None:
            validated_val_ids = current_ids

    if validated_val_ids is None:
        raise RuntimeError("No centralized validation IDs were verified.")

    # The centralized validation and test memberships are identical across
    # scaling points, so each dataset is loaded only once.
    validation_loader, validation_info = load_fedadam_validation_split(
        parquet_path=args.data_parquet,
        stats_path=args.norm_stats,
        row_ids=validated_val_ids,
