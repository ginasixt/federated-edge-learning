#!/usr/bin/env python3
"""
Threshold-dependent FedProx evaluation for validation-AP-selected checkpoints.
...
1. Loads the checkpoint already selected by highest validation Average Precision
   from:
       result/splits_iid_scaling/
         splits_iid_<N>_clients.json/FedProx/bestPRROC/run_<r>/
...
Complete validation-set handling
--------------------------------
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Select exact validation thresholds for FedProx bestPRROC "
...
def discover_best_pr_checkpoint(
    ...
    # Safety check: the AP-selected checkpoint must have been selected on the complete validation set.
...
def main() -> None:
    ...
    print("=" * 84)
    print("THRESHOLD-DEPENDENT FedProx EVALUATION")
    ...
    # Verify every split before evaluating any model. All validation IDs under
    # split_data["val"] are combined and must exactly match norm_stats["val_idx"].
    for n_clients in args.scaling_points:
        print(f"\n🔎 Verifying {n_clients} clients")
        validation_ids_from_split(
