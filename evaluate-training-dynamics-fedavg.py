#!/usr/bin/env python3
"""
Retrospective Test-Set Training-Dynamics Evaluation for FedProx
================================================================

For every FedProx scaling point, run, and saved communication-round
...
Late-training behavior
    ...
    the final ``LATE_ROUNDS`` communication-round values. With 80 total rounds
    and the default LATE_ROUNDS=11, the window is exactly rounds 70-80. These
    11 AP observations span 10 round-to-round intervals.
...
result/splits_iid_scaling/
  splits_iid_<N>_clients.json/
    FedProx/
      all_rounds_FedProx_<r>/
        model_round_<ROUND>*.pt

The older directory schemes ``all_rounds_run_<r>`` and ``all_rounds_<r>``
are also supported.
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Evaluate all saved FedProx checkpoints on the centralized test "
    ...
    parser.add_argument(
        ...
        default=11,
        help=(
            "Number of consecutive final round values used for the OLS trend. "
            "Use 11 to analyze rounds 70-80 inclusive when the final round is 80."
        ),
    )
    parser.add_argument(
...
def discover_run_models(
    ...
    """
    Locate one FedProx run across the directory schemes used in the project.

    Preferred FedProx layout:
        all_rounds_FedProx_<run>

    Older layouts remain supported:
        all_rounds_run_<run>
        all_rounds_<run>

    If more than one directory exists, unique communication rounds are merged.
    A duplicated round across directories is rejected to avoid evaluating an
    ambiguous checkpoint trajectory.
    """
    candidates = (
        strategy_dir / f"all_rounds_FedProx_{run_tag}",
...
def main() -> None:
    ...
    print("=" * 88)
    print("RETROSPECTIVE FEDPROX TEST-SET TRAINING-DYNAMICS EVALUATION")
    print(f"Strategy       : {args.strategy}")
    print(f"Scaling points : {args.scaling_points}")
    print(f"Runs           : {args.runs}")
    print(f"AP fraction    : {args.ap_fraction:.4f}")
    print(
        f"Late window    : final {args.late_rounds} round values "
        f"({args.late_rounds - 1} round-to-round intervals)"
    )
    ...
    write_json(summary_path, {
        ...
        "late_training_definition": (
            ...
            f"{args.late_rounds} communication-round values; with an 80-round "
            f"run and late_rounds=11, this corresponds to rounds 70-80 inclusive."
        ),
        "test_set": test_info,
