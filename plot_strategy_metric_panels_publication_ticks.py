...
def plot_family_panels(
    ...
    """Create a clean MCC + threshold figure and save it as PNG and PDF."""
    ensure_outdir(outdir)
    if family_df.empty:
    ...
    fig, (main_ax, threshold_ax) = plt.subplots(
        ...
        sharex=True,
        gridspec_kw={"height_ratios": [3.2, 1.15], "hspace": 0.10},
    )
    ...
    for variant in variants:
        ...
        main_ax.plot(
            ...
            group["best_mcc_value"].astype(float),
            color=color,
            linewidth=1.0,
            alpha=0.95,
            marker="o",
        ...
        threshold_ax.plot(
            group["round"].astype(int),
            group["best_mcc_threshold"].astype(float),
            color=color,
            linewidth=1.0,
            alpha=0.9,
            marker="s",
        ...
        if run_summary.empty:
            ...
            label = (
                f"Run {variant}: plateau {row['plateau_mcc_mean']:.3f} "
                f"± {row['plateau_mcc_std']:.3f}, R{target_pct:.0f}={convergence}"
            )
    ...
    main_ax.set_title("Best MCC per Round", fontsize=15, fontweight="bold")
    main_ax.set_ylabel("MCC", fontsize=12)
    format_panel(main_ax, family)

    threshold_ax.set_ylabel("Decision Threshold", fontsize=12)
    threshold_ax.set_xlabel("Communication Round", fontsize=12)

    # The evaluated decision thresholds lie on a fixed 0.05 grid. Show this
    # grid explicitly on the y-axis instead of using strategy-specific spacing.
    threshold_tick_interval = 0.05
    threshold_values = family_df["best_mcc_threshold"].astype(float).to_numpy()
    finite_thresholds = threshold_values[np.isfinite(threshold_values)]
    if finite_thresholds.size:
        threshold_min = np.floor(finite_thresholds.min() / threshold_tick_interval) * threshold_tick_interval
        threshold_max = np.ceil(finite_thresholds.max() / threshold_tick_interval) * threshold_tick_interval
        if np.isclose(threshold_min, threshold_max):
            threshold_min = max(0.0, threshold_min - threshold_tick_interval)
            threshold_max = min(1.0, threshold_max + threshold_tick_interval)
        threshold_ax.set_ylim(max(0.0, threshold_min), min(1.0, threshold_max))

    threshold_ax.yaxis.set_major_locator(mticker.MultipleLocator(threshold_tick_interval))
    threshold_ax.yaxis.set_major_formatter(mticker.FormatStrFormatter("%.2f"))
    threshold_ax.grid(axis="y", which="major", alpha=0.24)
    threshold_ax.grid(axis="x", which="major", alpha=0.12)
    ...
    for axis in (main_ax, threshold_ax):
        ...
        axis.tick_params(axis="x", which="major", labelsize=10.5)
        axis.tick_params(axis="y", which="major", labelsize=10.5)
        axis.tick_params(axis="x", which="minor", length=2, color="0.55")
    ...
    fig.suptitle(
        f"{family}: MCC Performance and Selected Decision Threshold",
        fontsize=17,
        fontweight="bold",
        y=0.995,
    )
    fig.subplots_adjust(left=0.10, right=0.985, top=0.92, bottom=0.17)
