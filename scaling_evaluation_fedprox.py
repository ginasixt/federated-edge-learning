...
FedProx Scaling-Study Evaluation
=================================
...
Bei den für FedProx verwendeten Split-Dateien liegt das vollständige\nValidierungsset unter:
    split_data["val"]["0"]
...
STRATEGY = "FedProx"          # Unterordner mit den Runs
N_RUNS = 5                    # z. B. all_rounds_FedProx_1 .. _5
IN_DIM = 21                   # Feature-Dimension (server_app: model_dim = 21)
BATCH_SIZE = 256             # Batchgröße der nachträglichen Val-Evaluation
USE_FOCAL = False            # FedProx-Training nutzte gewichtete CrossEntropy
FOCAL_GAMMA = 2.0
...
def split_json_path(n_clients: int) -> Path:
    """Pfad zur Split-Datei ``splits_iid_<N>_clients.json``."""
...
def build_val_loader(n_clients: int) -> Tuple[Optional[DataLoader], Optional[torch.Tensor]]:
    ...
    In den für FedProx verwendeten Split-Dateien liegt das vollständige
    Validierungsset im Eintrag
    ...
    # Sicherheitsprüfung: Keine in val["0"] aufgeführte Zeile darf bei der
    # nachträglichen zentralisierten Evaluation verloren gehen.
    expected_val_samples = len(val_row_ids)
    if X_val.shape[0] != expected_val_samples or len(y_val) != expected_val_samples:
        raise ValueError(
            f"Validation-Set unvollständig geladen: erwartet "
            f"{expected_val_samples} IDs aus val['{VAL_CLIENT_ID}'], "
            f"aber X_val={X_val.shape[0]} und y_val={len(y_val)}."
        )
...
def discover_round_models(run_dir: Path) -> List[Tuple[int, Path]]:
    ...
    NICHT aus dem Dateinamen. Unterstützt werden durch process_run() die
    Schemata all_rounds_FedProx_<r>, all_rounds_run_<r> und all_rounds_<r>.
    Der Suffix
    im Dateinamen
...
def discover_run_models(base: Path, run_tag: int) -> Tuple[List[Tuple[int, Path]], List[Path]]:
    """
    Sucht einen Run in allen verwendeten FedProx-Ordnerschemata:

      1. all_rounds_FedProx_<r>
      2. all_rounds_run_<r>
      3. all_rounds_<r>

    Falls mehrere Ordner existieren, werden eindeutige Runden zusammengeführt.
    Bei doppelten Rundennummern gilt die oben aufgeführte Priorität.
    """
    candidates = [
        base / f"all_rounds_FedProx_{run_tag}",
...
def main() -> None:
    print("=" * 78)
    print("🚀 FEDPROX SCALING-STUDY EVALUATION")
