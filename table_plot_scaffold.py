...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            ...
            "threshold-independent SCAFFOLD test performance."
        )
    )
    parser.add_argument(
        "--input",
...
def plot_table(
    ...
    ax.text(
        0.0,
        1.03,
        f"Run-to-Run Dispersion in Threshold-Independent {strategy} Test Performance",
    ...
    note = (
        f"Note. Statistics are based on five repeated runs at each of "
        f"{expected_scaling_points} scaling "
...
def write_dispersion_figure_caption(
    output_dir: Path,
    strategy: str,
    expected_scaling_points: int,
) -> Path:
    ...
    caption = f"""Figure A.X
Run-to-Run Dispersion in Threshold-Independent {strategy} Test Performance Across Client Scaling Points
...
def write_outputs(
    ...
    caption_path = write_dispersion_figure_caption(
        output_dir,
        strategy=strategy,
        expected_scaling_points=expected_scaling_points,
    )
    ...
    note = (
        f"Note. Statistics are based on five repeated runs at each of "
        f"{expected_scaling_points} scaling "
        "points. Mean SD is the arithmetic mean of the point-specific sample standard "
    ...
    markdown = (
        "**Table X**\n\n"
        f"*Run-to-Run Dispersion in Threshold-Independent {strategy} "
        "Test Performance*\n\n"
...
def main() -> None:
    ...
    print("=" * 78)
    print(f"{args.strategy.upper()} RUN-TO-RUN DISPERSION TABLE")
