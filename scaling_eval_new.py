...
def compute_curves(probs: np.ndarray, y: np.ndarray) -> Dict:
    """
    Vollständige ROC- und PR-Kurven (neu auf Val-Set) aus probs/y.
    JSON-serialisierbar, enthält alle Punkte zum Plotten + AUC-Werte.
    """
    if len(np.unique(y)) < 2:
        return {
            "roc_auc": float("nan"),
    ...
    fpr, tpr, roc_thr = roc_curve(y, probs)
    roc_auc = float(roc_auc_score(y, probs))

    precision, recall, pr_thr = precision_recall_curve(y, probs)
    pr_auc = float(average_precision_score(y, probs))
    ...
    return {
        ...
        "roc_curve": {
            ...
            "thresholds": [float(v) for v in roc_thr],
        },
        "pr_curve": {
...
def discover_round_models(run_dir: Path, run_tag: int) -> List[Tuple[int, Path]]:
    ...
    (all_rounds_run_<r>), NICHT aus dem Dateinamen. Der Suffix im Dateinamen
